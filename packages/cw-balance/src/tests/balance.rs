...
#[test]
fn test_operations_with_empty_vectors() {
    ...
    let result_add = balance1.checked_add(&balance2).unwrap();
    assert_eq!(result_add.native.as_ref().unwrap().len(), 1);
    assert!(result_add.cw20.is_none());
    assert!(result_add.cw721.is_none());

    let result_sub = result_add.checked_sub(&balance1).unwrap();
    assert_eq!(result_sub.native.unwrap().len(), 1);
    assert!(result_sub.cw20.is_none());
    assert!(result_sub.cw721.is_none());
}

#[test]
fn test_checked_sub_with_nonexistent_cw721() {
    let balance1 = BalanceVerified {
        native: None,
        cw20: None,
